 ...
 */
   ...
   public function order(){
      $order = M('order')->order('is_pay asc,order_time desc')->select();
      for($i = 0;$i<count($order);$i++)
      {
         $order[$i]['order_time']=date('Y-m-d H:i:s',$order[$i]['order_time']);
         $order[$i]['pay_time']=date('Y-m-d H:i:s',$order[$i]['pay_time']);
         $order[$i]['add_time']=date('Y-m-d H:i:s',$order[$i]['add_time']);
      }
